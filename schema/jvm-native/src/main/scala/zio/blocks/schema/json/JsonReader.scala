...
final class JsonReader private[json] (
  ...
  /**
   * Throws a [[JsonError]] indicating that a required field with the given name
   * is missing.
   *
   * @param reqField
  ...
  /**
   * Throws a [[JsonError]] indicating that a field with the given name is
   * duplicated.
   *
   * @param len
  ...
  /**
   * Throws a [[JsonError]] indicating that an unexpected field with the given
  ...
  /**
   * Throws a [[JsonError]] indicating that an illegal discriminator field name
   * was encountered.
   */
  def discriminatorError(): Nothing = decodeError("illegal discriminator")

  /**
   * Throws a [[JsonError]] indicating that an illegal value was encountered for
   * the given discriminator field.
   *
  ...
  /**
   * Throws a [[JsonError]] indicating that an illegal enum value with the given
   ...
   * default value or throws a [[JsonError]] if the provided default value is
   * `null`.
   *
   * @param default
   ...
   * value returns the provided default value or throws a [[JsonError]] if the
   * provided default value is `null`.
   *
   * @param default
  ...
  /**
   * Finishes reading the `null` JSON value and returns the provided default
   * value or throws [[JsonError]] with a message of expecting `null` or the
   * provided token.
   *
   * @param default
   *   the default value to return
  ...
  /**
   * Throws a [[JsonError]] wrapping the given error and adding a span.
   *
   * @param span
  ...
  def decodeError(span: DynamicOptic.Node, error: Throwable): Nothing = error match {
    case e: JsonError =>
      throw e.copy(path = DynamicOptic(span +: e.path.nodes))
      throw JsonError(error.getMessage, DynamicOptic(Vector(span)))
  }

  /**
   * Throws a [[JsonError]] wrapping the given error and adding two spans.
   *
   * @param span1
  ...
  def decodeError(span1: DynamicOptic.Node, span2: DynamicOptic.Node, error: Throwable): Nothing = error match {
    case e: JsonError =>
      throw e.copy(path = DynamicOptic(span1 +: span2 +: e.path.nodes))
    case _ => throw JsonError(error.getMessage, DynamicOptic(Vector(span1, span2)))
  }

  /**
   * Throws a [[JsonError]] wrapping the given error and adding a list of spans.
   *
   * @param spans
  ...
  def decodeError(spans: List[DynamicOptic.Node], error: Throwable): Nothing = error match {
    case e: JsonError =>
      throw e.copy(path = DynamicOptic(spans.foldLeft(e.path.nodes)((ss, s) => s +: ss)))
    case _ => throw JsonError(error.getMessage, DynamicOptic(spans.toVector))
  }
